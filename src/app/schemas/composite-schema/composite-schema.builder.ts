...
export class SchemaBuilderResolver {
    ...
    constructor(@Inject(SCHEMA_BUILDER_PROVIDER) providers: SchemaBuilderProvider[], private injector: Injector) {
        this.providers = providers.reduce((acc, provider) =>
            acc.set(provider.type, provider.schemaBuilder), new Map()
        );
    }
...
export class CompositeSchemaBuilder {
    ...
    extract(schema: AbstractControlSchema, rawValue: any): any {
        ...
        if (schema instanceof FormGroupSchema) {
            ...
            const item = Object.keys(rawValue).reduce((acc, key) =>
                ({ ...acc, [ key ]: this.extract(schema.controls[ key ], rawValue[ key ]) }), {}
            );
